...
import infoEmbed from './InfoEmbeds'
...
export const SelfBotInfo = async (ctx: ChatInputCommandInteraction) => {
    ...
    await ctx.editReply({ embeds: [infoEmbed(ctx.client)] }) 
...
export const GuildInfo = async (ctx: ChatInputCommandInteraction) => {
...
export const PlayerInfo = async(ctx:ChatInputCommandInteraction) => {
  await ctx.editReply('製作中...')
}